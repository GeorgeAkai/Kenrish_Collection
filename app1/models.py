...
class Product(models.Model):
    ...
    average_rating = models.FloatField(default=0.0)  # ✅ Changed from @property to model field
...
class Handbag(models.Model):
    ...
    created_at = models.DateTimeField(auto_now_add=True)
    average_rating = models.FloatField(default=0.0)  # <-- ADD THIS FIELD
...
class InventoryTransaction(models.Model):
    ...
    def __str__(self):
        item = self.product or self.handbag
        return f"{self.get_transaction_type_display()} - {item.name} ({self.quantity})"
...
class Sale(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if is_new:
            CashFlow.objects.create(
                ...
                description=f"Sale: {(self.product or self.handbag).name} x{self.quantity}",
                reference_sale=self,
    ...
    def __str__(self):
        item = self.product or self.handbag
        return f"Sale - {item.name} x{self.quantity}"
# Add inventory properties to Product and Handbag models
