  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      <div className="mb-6 sm:mb-8">
        <h1 className="section-heading">Clothes</h1>
      ...
      {loading ? (
        ...
        <>
          <div className="product-grid">
            {data?.results.map(c => (
              <CatalogueCard key={c.id} item={c} href={`/clothes/${c.id}`}
                inWishlist={isInWishlist('clothes', c.id)} onWishlist={() => toggle('clothes', c.id)} />
            ))}
          </div>
          {data?.count === 0 && <p className="text-center text-muted-foreground py-20">No clothes available yet.</p>}
